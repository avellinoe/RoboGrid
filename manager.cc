...
namespace elma {
    ...
    // Manager& Manager::watch(std::string event_name, std::function<void(Event&)> handler) { // original
    // Manager& watch(string, std::function<void(Event&)>, int) { // 3 args, default
    Manager& Manager::watch(string event_name, std::function<void(Event&)> handler, int priority/*=0*/) { // priority as default
        auto handler_priority = std::make_tuple(handler, priority);
        // std::cout << "Adding " << event_name << " handler with priority: " << std::get<1>(handler_priority) << "\n";
        event_handlers[event_name].push_back(handler_priority);
        // if (event_handlers[event_name].size() == 1) {
        //     std::cout << "There is currently 1 handler for " << event_name << "\n";
        // } else {
        //     std::cout << "There are currently " << event_handlers[event_name].size() << " handlers for " << event_name << "\n";
        // }
        return *this;
    }
    ...
    Manager& Manager::emit(const Event& event) {
        ...
        std::tuple<std::function<void(Event&)>, int> top_priority_handler; // this is the handler that'll run
        std::get<1>(top_priority_handler) = lowest_priority;
            /* with the default priority set to zero, we assume that the user would have */

        if ( event_handlers.find(event.name()) != event_handlers.end() ) {
            // std::cout << "grabbing handler for " << event.name() << " event.\n";
            for ( auto handler_priority : event_handlers[event.name()] ) {
                // std::cout << "top priority: " << std::get<1>(top_priority_handler) << " incoming priority: " << std::get<1>(handler_priority) << "\n";
                if (std::get<1>(handler_priority) < std::get<1>(top_priority_handler)) {
                    std::get<0>(top_priority_handler) = std::get<0>(handler_priority);
                    std::get<1>(top_priority_handler) = std::get<1>(handler_priority);
                }
            }
            if ( e.propagate() ) {
                std::get<0>(top_priority_handler)(e);
            }
        } else {
            string e = "There are no handlers for the event: ";
            e.append(event.name());
            e.append("");
            throw Exception(e);
        }
