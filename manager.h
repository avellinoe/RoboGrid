...
namespace elma {
    ...
    class Manager {
        ...
        // Event Interface
        // Manager& watch(string event_name, std::function<void(Event&)> handler); // original
        // Manager& watch(string, std::function<void(Event&)>, int); // 3 args, default
        Manager& watch(string event_name, std::function<void(Event&)> handler, int priority=0); // priority as default
        ...
        map<string, vector<std::tuple<std::function<void(Event&)>, int>>> event_handlers;
